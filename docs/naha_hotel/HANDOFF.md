...
### 디자인 (2026-08-17 모바일 개편)

읽는 상황이 "밤에 폰으로, 며칠에 걸쳐 반복해서"라 모바일 우선으로 다시 짰다.

- **시그니처는 3분 선이다.** 모든 행이 0–10분 도보 척도를 공유하고 30% 지점(3분)에 세로 눈금이
  선다. 눈금이 세로로 이어져 목록을 관통하는 선처럼 보이고, 훑어보기가 "선 왼쪽 점 찾기"가 된다.
  3분은 임의 수치가 아니라 첫 번째 필터이자 이 문서가 밝힌 최우선 기준이다.
  - 정렬이 어긋나지 않으려면 **머리글(`.scanhead`)과 행(`.scanrow`)이 같은 격자여야 하고
    3열은 고정폭(92px)이어야 한다.** `auto`는 격자마다 따로 계산돼 눈금이 어긋난다.
- **`<768px`는 자 목록, `≥768px`는 기존 표.** 예전엔 모바일에서도 9열 820px 표를 가로로 밀어야 해서
  요금과 도보시간을 동시에 볼 수 없었다 — "한눈에 보기"가 한눈에 안 보였다.
- **팔레트는 류큐 빙가타(紅型)의 주홍·호박·쪽빛.** 겸사겸사 인코딩 버그를 고쳤다: 예전 척도가
  파랑→주황→**초록**이라 가장 먼 9분 이상이 초록(= 좋음)으로 읽혔다. 지금은 가까울수록 뜨겁다.
  `--s1`(3분 이내) → `--s2`(4~8분) → `--s3`(9분+) 순서는 지도 핀·차트·표가 공유하므로 바꾸지 말 것.
- **상시 하단 바를 없앴다.** 76px + 안전영역을 늘 먹고 있었다. 테마 토글은 헤더로, 맨 위로는
  *위로 스크롤할 때만* 뜨는 작은 버튼(`.totop`)으로 옮겼다 — 고정 버튼이 카드 금액을 가리지 않는다.
- 한국어는 `word-break: keep-all`이 필수다. 없으면 제목이 "남겼습니 / 다"로 끊긴다.
